...
void CDownloadQueue::LogDownloadSlotDiagnostics(ULONGLONG curTick) const
{
	...
	UINT uDuplicateZeroWriteSources = 0;
	uint64 uDuplicateZeroWritePackets = 0;
	uint64 uDuplicateZeroWriteBytes = 0;
	...
	for (POSITION pos = filelist.GetHeadPosition(); pos != NULL;) {
		...
		for (POSITION sourcePos = cur_file->srclist.GetHeadPosition(); sourcePos != NULL;) {
			const CUpDownClient *cur_source = cur_file->srclist.GetNext(sourcePos);
			if (!cur_file->IsLiveSource(cur_source))
				continue;
			const ULONGLONG ullSourceDuplicatePackets = cur_source->GetDownloadDuplicateZeroWritePackets();
			if (ullSourceDuplicatePackets > 0)
				++uDuplicateZeroWriteSources;
			uDuplicateZeroWritePackets += ullSourceDuplicatePackets;
			uDuplicateZeroWriteBytes += cur_source->GetDownloadDuplicateZeroWriteBytes();
		}
...
{
	if (sender->IsStopped()) {
		UpDownClientDeleteSeams::AssertTemporarySourceReadyToDelete(source, sender, _T("CDownloadQueue::CheckAndAddSource stopped file"));
		delete source;
		return false;
	}
	...
	if (source->HasValidHash() && md4equ(source->GetUserHash(), thePrefs.GetUserHash())) {
		...
		UpDownClientDeleteSeams::AssertTemporarySourceReadyToDelete(source, sender, _T("CDownloadQueue::CheckAndAddSource own hash"));
		delete source;
		return false;
	}
	...
	if (theApp.clientlist->m_globDeadSourceList.IsDeadSource(*source) || sender->m_DeadSourceList.IsDeadSource(*source)) {
		...
		UpDownClientDeleteSeams::AssertTemporarySourceReadyToDelete(source, sender, _T("CDownloadQueue::CheckAndAddSource dead source"));
		delete source;
		return false;
	}
	...
	for (POSITION pos = filelist.GetHeadPosition(); pos != NULL;) {
		...
		for (POSITION pos2 = cur_file->srclist.GetHeadPosition(); pos2 != NULL;) {
			...
			if (cur_client->Compare(source, true) || cur_client->Compare(source, false)) {
				...
				UpDownClientDeleteSeams::AssertTemporarySourceReadyToDelete(source, sender, _T("CDownloadQueue::CheckAndAddSource duplicate source"));
				delete source;
				return false;
			}
		}
