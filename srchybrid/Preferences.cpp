...
CString ReadStartupConfigProfileDirectoryPreference(EDefaultDirectory eDirectory)
{
	...
	if (!LongPathSeams::PathExists(strPreferencesPath))
		return CString();
...
bool	CPreferences::m_bDownloadAutoBroadbandIO;
...
void CPreferences::SavePreferences()
{
	...
	ini.WriteBool(prefini::DownloadIoKeys::DownloadAutoBroadbandIO, m_bDownloadAutoBroadbandIO);
...
void CPreferences::LoadPreferences()
{
	...
	m_bDownloadAutoBroadbandIO = ini.GetBool(prefini::DownloadIoKeys::DownloadAutoBroadbandIO, true);
	...
	m_iExtractMetaData = ini.GetInt(_T("ExtractMetaData"), 1); // 0=disabled, 1=auto media metadata; legacy 2 normalizes to 1
...
CString CPreferences::GetDefaultDirectory(EDefaultDirectory eDirectory, bool bCreate)
{
	if (m_astrDefaultDirs[0].IsEmpty()) { // already have all directories fetched and stored?
		...
		m_astrDefaultDirs[EMULE_SKINDIR] = m_astrDefaultDirs[EMULE_EXECUTABLEDIR] + _T("skins\\");
